# lintal: Java Linter with Auto-Fix
...
│   ├── lintal_source_file/     # Locator, line indexing (from Ruff)
...
Parsed directly from existing files. Module names map to lintal rules:
...
**Merge order:** checkstyle.xml defines *what* rules run and their parameters. lintal.toml defines *how* violations are handled.
...
# Check files
lintal check src/
lintal check src/main/java/com/example/

# Fix files
lintal fix src/
lintal fix src/ --unsafe      # Include unsafe fixes
lintal fix src/ --diff        # Preview without applying

# Filter rules
lintal check src/ --select WhitespaceAround,LeftCurly
lintal check src/ --ignore FinalParameters

# Config
lintal check src/ --config path/to/checkstyle.xml

# Output formats
lintal check src/ --output-format text
lintal check src/ --output-format json
lintal check src/ --output-format github
```
...
- Build lintal_java_cst with typed node wrappers
- Basic CLI: `lintal check <path>` with single hardcoded rule
- Verify end-to-end: parse → check → report

### Phase 2: Config System
- lintal_checkstyle crate: parse checkstyle.xml
- Map module names to rule enum variants
- Extract properties into typed config structs
- Add lintal.toml overlay parsing
